...
**One** parameterized package for all customers and all platforms. It ships
once; the only per-customer act is supplying parameter values via an **SSISDB
environment** (generated by VALC, written + bound by the agent over T-SQL).
`ProtectionLevel = DontSaveSensitive` so the `.ispac` carries no secrets and is
identical everywhere. Adding a JDE column = rebuild one `.ispac` → catalog it in
VALC → agent redeploys to everyone. No customer-box Visual Studio, no manual
package edits per install.
...
| `DecExtCost` | int | **derived** `10^clients.jde_ecst` | count→10ⁿ |
| `DecUnitCost` | int | **derived** `10^clients.jde_uncs` | |
| `DecQty` | int | **derived** `10^clients.jde_pqoh` | |
| `DecQtyCX` | int | **derived** `10^clients.jde_trqt` | |
