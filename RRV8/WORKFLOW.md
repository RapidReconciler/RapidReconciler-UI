...
longer needed): `STRING_SPLIT`, `STRING_AGG`, `TRIM`,
`CONCAT`/`CONCAT_WS`, `IIF`, `OFFSET/FETCH NEXT`,
`TRY_CAST`/`TRY_CONVERT`/`TRY_PARSE`, `LAG`/`LEAD`/`FIRST_VALUE`,
`SUM(x) OVER (ORDER BY y)` cumulative, JSON (`OPENJSON`, `JSON_VALUE`,
