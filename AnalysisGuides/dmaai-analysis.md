...
|---|---|---|---|
| **4122** | IA, II, IJ, IL, IM, IP, IR, IV | Inventory leg | Primary inventory account for adjustments (IA), internal transfers (II), physical inventory adjustments (IJ), lot transfers (IL), material issues (IM), physical inventory (IP), **reclassifications (IR)** and voids (IV). This is the most critical inventory table. ⚠ `IR` read "receipts" here until 2026-09-16; Oracle's Inventory Management *Document Type* page names it **Inventory Reclassification**, and neither specimen database holds an `IR` item-ledger row to test the behaviour against (control: `IA` non-zero on both), so the correction rests on that page. Also the inventory relief on a **maintenance work order** issue (order type `WM`), paired with 4124 — see the note below. |
