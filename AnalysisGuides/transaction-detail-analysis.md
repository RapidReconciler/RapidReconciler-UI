...
### 0.4 `IQ` is Advanced Warehousing, and it CANNOT produce a variance

**Document type `IQ` is a pick or putaway request generated by the Advanced Warehousing
module** (owner, SME, 2026-09-16). It is the largest document population in the fleet
and it had no entry anywhere in this catalog until now, which is worth saying plainly:
477,116 F4111 rows on one specimen database, more than any other document type there.

**It moves quantity between locations and moves no value.** Measured on that database:

| | `IQ` | control: `IA` |
|---|---|---|
| F4111 rows | 477,116 | 114,825 |
| distinct locations | 4,281 | — |
| rows with quantity **> 0** | **238,558** | 111,692 |
| rows with quantity **< 0** | **238,558** | 3,133 |
| documents carrying **both** signs | **35,047 of 35,047** | 216 of 111,627 |
| rows carrying a non-zero amount | **160 of 477,088** | 11,503 of 114,033 |
| gross value on those 160 rows | 23,351.38, **netting to 0.00** | 19,061,087.60 |

Every `IQ` document, without exception, takes quantity out of one location and puts the
same quantity into another. The perfectly matched ± counts and the 100% both-signs
figure are what a location-to-location move looks like in the item ledger; the control
is lopsided, exactly as an adjustment should be. **Inventory value does not change, so
there is no journal entry to find.**

**It carries no JDE batch either** — `ilicu = 0` on every one of the 477,116 rows, zero
exceptions. RapidReconciler synthesises batch **99999999** for them
(`v6_006_trans_new_rows`), which is what lets them past the `Batch > 0` filter in
`v6_008_reconcile`; that view then stamps batch type **`aw`** and types the row
**`Warehouse`**. None of that was written down anywhere before this section.

**So `Type = 'Warehouse'` holds zero rows, by construction rather than by luck.**
`v6_008_reconcile` only emits a row whose absolute cardex-minus-ledger difference
exceeds the company threshold. For `IQ` both sides are 0.00, so the difference is 0.00
on **every one** of the 35,040 compare rows — measured, against a control of 18,064
differing `IA` rows on the same database. Zero reach `RCardexLedgerCompare2`, so zero
reach a card.

**What this means for analysis**

1. **Do not chase a missing GL entry on an `IQ` document.** There is nothing to post.
   A pick or putaway request is not an inventory transaction in the accounting sense.
2. **`IQ` presence is the Advanced Warehousing signal.** Two of the three specimen
   databases hold **zero** `IQ` rows (control: `IA` non-zero on both), and the third
   holds 477,116. If a client has no `IQ`, they are not running the module.
3. **A non-zero `IQ` amount is worth a second look.** 160 rows out of 477,088 carried
   one, and they net to zero across the population. A valued warehouse movement is rare
   enough to be interesting rather than routine.

**Sourcing, stated honestly.** The document-type code `IQ` is **not** in the public
Oracle documentation I could reach — the Inventory Management *Document Type* page
lists `IM`, `IC`, `IS`, `OV`, `RI`, `ST`, `OT` and `IR` and does not mention it, and
`00/DT` is customer-configurable while RapidReconciler does not extract `F0005`. What
Oracle *does* confirm is the mechanism: putaway and pick confirmation update the Item
Ledger `F4111` alongside the warehouse request, suggestion and task tables (`F4600`,
`F4611`, `F4601`). The code itself rests on the owner's SME reading plus the
measurements above, which corroborate it independently.

### 0.5 The shape test: a MOVEMENT pairs, an ADJUSTMENT does not

**Read the quantity signs before reading anything else.** Two questions settle what a
document type is, and both are one query against `F4111`: do its rows pair off + against
−, and does value travel with them. Measured across the three specimen databases:

| Doc type | rows | qty **+** | qty **−** | docs with both signs | rows with value | net value |
|---|---|---|---|---|---|---|
| `IQ` warehouse move | 477,116 | 238,558 | 238,558 | **35,047 of 35,047** | 160 | **0.00** |
| `IK` reclassification | 74,997 | 37,502 | 37,495 | — | 46,573 | **9,951.43** of 65.9M gross |
| `PI` cycle count | 9,406 / 1,085 / 18,464 | 5,332 / 383 / 5,377 | 4,074 / 702 / 6,294 | — | most | +90,607 / −39,666 / −429,759 |
| `IA` adjustment (control) | 171,320 | 10,193 | 159,586 | 676 of 29,447 | 168,838 | −1,306,577 |

- **Paired counts and a net near zero ⇒ a MOVEMENT.** The same stock left one identity
  and arrived as another. `IQ` moves a *location* and carries no value at all; `IK` moves
  an *item* and carries large value that cancels — 65.9M gross netting to 9,951.43, which
  is 0.015%.
- **Lopsided counts and a real net ⇒ an ADJUSTMENT.** `PI` and `IA` swing either way by
  database, which is what counting stock and correcting it produces.

**The residual on a movement is the finding, and it is small by construction.** A
reclassification that does not net means the two legs were valued differently — the whole
point of the document is that they should not be. Do not work the 99.98% that tied.

**⚠ A pair can span TWO document types, and then the per-document test reads zero.**
`TR` is 28,489 rows and **every one is negative**; `OW` is 29,354 rows and **every one is
positive**. Neither has a single document carrying both signs, so the test above calls
them one-sided — yet they are the outbound and inbound legs of the same transfer order,
under order types `ST` and `OT`. Across one database `TR` nets −40,835,422 against `OW`
+40,551,483.

So when a type comes back strictly one-directional, **look for its counterpart before
concluding anything**. Join the two on order number rather than trusting the totals:
491 orders carry both legs there, out of 1,234 `TR` and 1,207 `OW` distinct orders, and
those 491 net to −444,724 on about 16M. ⚠ **That leaves roughly 60% of orders with one
leg only in this data, and why is NOT established** — it may be the load window, it may
be real. Do not present the pairing as tight.

**And run the value half at the right grain.** The quantity questions read `F4111`,
which is per-transaction and correct as written. The value and break questions must
**not** be read off raw `RCardexLedgerCompare` rows — that table holds each leg
separately, so every row looks one-sided. Group first. §0.7 is what happens when you
skip that, written up as a retraction.

### 0.6 `PI` is a cycle count AND this fleet's AAI keying doc type. Two different things.

`PI` is a **cycle count** (owner, SME, 2026-09-16) — it appears on all three specimen
databases, unlike `IQ`, so counting is universal here where Advanced Warehousing is not.

⚠ **It is also the value of `RCompanies.AAIDocType` on two of the three**, which is what
puts `Ty PI` on every `4152 Inv Model` row in the DMAAI block of a Transaction Detail
report — on documents that have nothing to do with counting. **The third database proves
they are independent: its `AAIDocType` is `99` while it still carries 9,406 `PI` count
transactions.** Reading `Ty PI` in the model block as evidence of a cycle count is
therefore wrong, and the two roles have to be held apart.

### 0.7 ⛔ NEVER COUNT BREAKS ON RAW `RCardexLedgerCompare` ROWS. Group first.

**This section said the opposite on 2026-09-16 and it was wrong. The retraction is the
lesson, so it stays.** It reported that `PI` and `IK` "break far more often than their
totals suggest" — 1,765 differing rows on one database, every one of 309 on another —
and read the near-equal cardex-only / GL-only counts as *"the signature of a match-key
failure"*. That would have sent someone hunting a correlation bug that does not exist.

**`RCardexLedgerCompare` holds the cardex leg and the GL leg as SEPARATE ROWS by
design.** Comparing them row by row makes every row one-sided and therefore "differing",
which measures the table's grain rather than the data. `v6_008_reconcile` groups by
batch, company, document type, document number and account *before* it compares, and
that is the only grain at which a difference means anything.

Re-measured at the comparison grain, the same populations:

| | groups | both sides | differing |
|---|---|---|---|
| `PI` (Demo3) | 1,159 | 882 | **0** |
| `PI` (Demo1) | 388 | 378 | **2** |
| `PI` (Demo2) | 155 | 154 | **1** |
| `IK` (Demo3) | 37,408 | 799 | **0** |
| `IA` control (Demo3) | 110,982 | 9,032 | **0** |
| `IA` control (Demo1) | 37,455 | 36,862 | 73 of 74,825 raw rows |

Cycle counts and reclassifications essentially do not break. The earlier figures were
the raw row counts of a table nobody should compare at that grain.

**How to tell the artefact from the finding.** After grouping, a genuinely one-sided
population stays one-sided: `UP` is 110 groups, **110 GL-only, 110 differing**, on the
one database that carries it. That is real, and it is what §5.2 is for.

    select  Batch, CompanyNumber, DocType, DocNumber, ShortAccount
    ,       sum(CardexAmount) as cx, sum(LedgerAmount) as gl
    from    RCardexLedgerCompare
    group by Batch, CompanyNumber, DocType, DocNumber, ShortAccount

Run that before quoting any break count off the compare.
...
| Cycle count update batch ran in a different period than the count entry | Doc type is **`PI`**; F4111 date and F0911 GL date are in different periods | Check the GL Date option in R41413/R41610. The count entry date and the update batch date spanned a period end. Post a manual journal entry to move the GL amount to the correct period. ⚠ **This row said "doc type is WK or WS" until 2026-09-16 and could never have fired**: neither exists as a document type on any specimen database (control: `PI` non-zero on all three, and `WS` returns 67,713 rows as an ORDER type on one). The same doc-type / order-type conflation that runs through Section 9. `PI` is the cycle count — see §0.6, and note it is also the AAI keying doc type on two databases. |
...
#### Maintenance WO returns are the same document type with the sign reversed

There is no separate return document. Material coming back off a maintenance work
order is an **`IM` row under order type `WM` with a positive quantity**, against the
negative quantity of the issue. Measured:

| | returns (qty > 0) | issues (qty < 0) |
|---|---|---|
| rows, one database | 8,310 | 208,500 |
| value | +18,080,983 | −222,950,663 |

Returns run about 4% of issues by count and 8% by value there, which is a normal
working rate rather than something to chase.

**⚠ A second database closes its maintenance work orders differently and it is worth
knowing before you diagnose one.** It carries **no** positive `IM` rows at all under
`WM`. Instead the issues (`IM`, −22,012,229) are offset by **`IC` rows** (+22,011,893)
— the two sides landing within **336.17** of each other across the whole population.
Its `WM` orders also carry a little `II` (140 issues) and `IA` (43 returns).

So "the return leg is missing" is not a finding until you have checked which shape
that client uses. One returns material; the other closes the order out through `IC`.
Why they differ is **not established** — it is a configuration or process question for
the customer, not something the data here answers.
...
| **PI** | Periods | Did the count update batch (R41413/R41610) run in a different period than the count entry date? Compare the GL Date option. ⚠ Keyed on `WK / WS` until 2026-09-16 and could never fire: neither exists as a document type on any specimen database, while `PI` does on all three. Second instance of the same row. | R41413 / R41610 GL Date |
...
## Section 9: Document Type and Order Type Reference

**⚠ This section carried both under one "Document Type" heading until 2026-09-16, and
about ten of its entries were order types.** Anyone keying a rule off it wrote
`DocType = 'ST'` and matched nothing — which is exactly what the cycle-count row in
Section 5 did with `WK`/`WS` for however long it stood there. The split below was
measured rather than reasoned: each code was counted in `F4111` as `ildct` and as
`ildcto` across all three specimen databases.

| Measured as | Codes |
|---|---|
| **document type only** | `IA` `IB` `IC` `II` `IM` `IQ` `IK` `ID` `IT` `JS` `OV` `PI` `PV` `RI` `RM` `TR` `OW` `WD` `FA` |
| **order type only** | `C2` `CW` `OT` `SK` `ST` |
| **both** — predominantly order type, but present as a document type on at least one database | `SI` (3,272 doc / 21,869 order), `SA` (20 / 5,583), `CO` (22 / 328), `SP` (2 / 106) |
| **neither — not present in any specimen** | `IH` `OK` `PD` |

`UP` is the exception to the whole section: it has **no item ledger at all**, so it is
a document type on the GL side only. See its row below.

### 9.1 Document types

| Document Type | Description |
|---|---|
| **OV** | Purchase order receipt (creates inventory debit via AAI 4310 and RNV credit via AAI 4320). A standard cost variance entry may appear in F0911 Exp Acct via AAI 4335 — this is not written to F4111 and is expected. For account mismatches, check P4312 PO 1 (Cost Method) — if the receipt cost differs from the F4105 standard, the variance posts to 4335; a misconfigured 4335 can produce an unexpected account. For period mismatches, check P4312 PO 2 (GL Date) — if set to use the PO promised date, a receipt may post to a prior period. **RTVs and receipt reversals live here too** — they are `OV` rows with a NEGATIVE quantity, not a document type of their own. Measured: 1,853 of 318,478 on the busiest order type of one database (0.58%), 463 and 33 on another, 65 on a third. Filter on the sign to isolate them. |
...
| **IQ** | **Advanced Warehousing pick or putaway request.** Moves quantity between locations and moves no value, so it has no GL leg and **cannot produce a variance** -- every document carries a matched + and - quantity pair, and both compare amounts are 0.00 on every row. Carries no JDE batch (`ilicu = 0`); RapidReconciler synthesises batch 99999999, stamps batch type `aw` and types it `Warehouse`. Do not chase a missing GL entry here. Full treatment and the measurements in §0.4. |
| **PI** | **Cycle count.** An adjustment, not a movement: quantity signs are lopsided and swing either way by database, with a real net. Present on every specimen database, so counting is universal here. Reaches the GL and breaks heavily -- on one database every one of its 309 compare rows differs. ⚠ **Do not confuse it with `RCompanies.AAIDocType`, which is also `PI` on two of three databases** and is what stamps `Ty PI` on the `4152 Inv Model` rows of unrelated documents. See §0.5, §0.6 and §0.7. |
| **IK** | **Item reclassification** -- stock leaves one item number and arrives as another. A movement: quantity pairs off (37,502 up against 37,495 down) and 65.9M of gross value nets to 9,951.43. AAI **4122** is configured for it. Overwhelmingly ties; 1,637 of 74,836 compare rows differ and every one of those is one-sided. The residual is the finding, not the 97.8% that tied. See §0.5 and §0.7. |
| **TR** | **Outbound leg of a transfer order**, under order type `ST`. Strictly one-directional: 28,489 item-ledger rows and **every one is negative**, netting −40,835,422. Its counterpart is `OW`. Breaks rarely once grouped — 85 of 2,566 groups. See §0.5 on cross-document-type pairs. |
| **OW** | **Inbound leg of a transfer order**, under order type `OT`. The mirror of `TR`: 29,354 rows, **every one positive**, netting +40,551,483. 75 of 2,679 groups differ. ⚠ `ST` and `OT` appear in this table as if they were document types; they are the ORDER types on these two. |
| **WD** | **Value-only document — quantity is zero on every row.** 5,193 item-ledger rows, all with a value, none with a quantity, netting +191,541. A revaluation or cost adjustment rather than a movement. Ties completely: 4,704 groups, **0 differing**. |
| **ID** | **Value-only document, one-directional.** 6,261 rows, quantity zero on every one, all positive, gross **106,009,758** — the largest value population in the fleet, and one row per document. Ties completely: 5,688 groups, **0 differing**. Nothing here needs working; know it exists so its size does not alarm anyone. |
| **FA** | Outbound relief under sales order type `S8`. 1,950 rows, all negative, netting −1,824,639. 207 groups, **0 differing**. |
| **JE** | Journal entry touching an inventory account. Behaves differently per database: outbound with an item-ledger leg on one (1,089 rows, all negative, order type `ST`), and **GL-only on another** — 69 groups, 50 GL-only, cardex 0.00 against ledger 1,588,652. Work the GL-only form as §5.2. |
| **UP** | **GL-side only. It has no item ledger at all** — zero `F4111` rows and zero `RTransactions` rows on every database (control: `IA` 171,320), while the compare holds 110 rows, all GL-only, ledger −266,470. The shape test cannot classify it because it has no shape in `F4111`. Treat as §5.2. |
| **IR** | Oracle's Inventory Management *Document Type* page names `IR` **Inventory Reclassification**, and it is presumably the shipped default. ⚠ **This fleet does not use it** -- zero `IR` item-ledger rows on any specimen database (control: `PI` and `IA` non-zero on all three), while `IK` carries the reclassification behaviour and is what you will actually meet. One database has AAIs configured for `IR` and no transactions against them. `00/DT` is customer-configurable, so expect the code to differ per client and read the data rather than the default. This row previously said "receipts", which was wrong. |

### 9.2 Order types

These sit in the **OrderType** column, not the document type column. Measured: none of
`C2`, `CW`, `OT`, `SK` or `ST` appears as a document type on any specimen database.

| Order Type | Description |
|---|---|
| **ST** | Transfer order sales (shipment). The document type carrying it is **`TR`**, which is strictly negative — see §0.5 on cross-document-type pairs |
| **OT** | Transfer order purchase (receipt). The document type carrying it is **`OW`**, strictly positive, and the counterpart of `TR` |
| **SI** | Intercompany sales order. ⚠ Also a document type on one database (3,272 rows) |
