...
> ⚠ **STALE, AND ONE OF ITS BULLETS IS KNOWN WRONG. Do not wire this file as-is.**
> The live grounding is generated by `Tools/build-ai-grounding.py` from
> `AnalysisGuides/_catalog/analyst/`, and that catalog has moved on. The A/P
> voucher bullet below still says the cause of voucher value on an inventory
> account *is* a DMAAI error and prescribes correcting AAI 4330. That is the
> STANDARD-COST branch only, and it is backwards on a weighted-average customer,
> where 4330 routes to inventory by design and the missing F4111 revaluation is
> the fault (measured on Demo2, 2026-09-12; see the catalog bullet and
> `transaction-detail-analysis.md` §5.15). Anything lifted from here has to be
> re-checked against the catalog first.
