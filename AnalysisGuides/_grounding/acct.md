...
- The out-of-balance decomposes into components. ACCOUNTANT-OWNED (journal these): carry forward, transactions, manual entries. NOT the accountant's: unposted GL batches + end-of-day (operations timing — self-clears when operations posts, never journal it) and cardex (analyst / operations work that RR's refresh recomputes, not the accountant's). The adjusting entry uses ONLY the accountant-owned amount; never journal the timing.
- Reclass vs JE: a transaction in the wrong period/account is a reclass, not a new balancing JE. A roll-forward break (red dot) is not the accountant's — it is analyst / operations maintenance with no manual lever: RR recomputes the full period timeline on every refresh, so it clears on the next run, and one that survives a refresh goes to the customer's IT department. Never prescribe a re-roll (retired).
